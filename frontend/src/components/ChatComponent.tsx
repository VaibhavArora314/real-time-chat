...
const ChatComponent = ({ sendMessage }: ChatComponentProps) => {
        ...
        <div className="bg-gray-100 flex flex-col p-4 h-[62vh] rounded-lg overflow-y-auto scrollbar">
          {room?.messages.map((message: MessageInteface) => {
            if (!message.sender || !message.sender._id)
              return (
                <div key={message._id} className={`mb-2 text-center break-words`}>
            ...
            if (message.sender._id == user._id)
              return (
                <div key={message._id} className={`mb-2 text-end break-words`}>
                  <p>{message.content}</p>
                </div>
              );

            return (
              <div key={message._id} className={`mb-2 text-start break-words`}>
                <p>
